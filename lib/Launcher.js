...
class Launcher {
  ...
  static async launch(options) {
    ...
    const profilePath = path.resolve(argv(chromeArguments.filter(arg => arg.startsWith('--user-data-dir')))['user-data-dir']);
    ...
    try {
      ...
      const browserWSEndpoint = await waitForWSEndpoint(chromeProcess, profilePath, options.timeout || 30 * 1000);
 ...
 */
function waitForWSEndpoint(chromeProcess, profilePath, timeout) {
  return new Promise((resolve, reject) => {
    ...
    let watcher = chokidar.watch(path.resolve(profilePath, 'DevToolsActivePort'))
      .on('add', onFile)
      .on('change', onFile);
    ...
    function onFile(path) {
      const match = fs.readFileSync(path, 'utf8').match(/^[0-9]+[\r\n]+.+$/m);
      if (!match) {
        return;
      }
      cleanup();
      resolve('ws://127.0.0.1:' + match[0].split(/[\r\n]+/).join(''));
    }
